...
Players.prototype.compareScores = function () {
  const awesomeOneInteger = Number($(".awesome-score-one").html());
  const awesomeTwoInteger = Number($(".awesome-score-two").html());

  if (awesomeOneInteger > awesomeTwoInteger) {
    $(".card-czar").children().hide();
    $(".card-czar").prepend(
      '<img class="win-img" src="http://1.media.collegehumor.cvcdn.com/78/41/085672321fd51c6034353cb12754397a-hans-solo-again.jpg"/>'
    );
    ...
    $(".card-czar").children().hide();
    $(".card-czar").prepend(
      '<img class="win-img" src="http://charts.stocktwits.com/production/original_52711289.png?1460436473"/>'
    );
